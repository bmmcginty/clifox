    ...
    def handleKey(self, c):
        ...
        if c == curses.KEY_DOWN:
            if self.controlIndex >= len(self.controls):
                self.beepIfNeeded()
                self.setStatus(
                    "No more controls in this dialog. Please up arrow to the first control.")
...
#prompt and delimiter
        self.s = u"%s%s" % (self.prompt, self.delimiter,
                            ) if self.prompt else ""
...
# move to the end of this line, history or tempLine
            ...
            if self.ptr > 0:
                self.currentLine = u"%s%s" % (
                    self.currentLine[:self.ptr-1], self.currentLine[self.ptr:])
                self.ptr -= 1
            else:
                self.beepIfNeeded()
            ...
            if self.ptr < len(self.currentLine):
                self.currentLine = u"%s%s" % (
                    self.currentLine[:self.ptr], self.currentLine[self.ptr+1:])
            else:
                self.beepIfNeeded()
        elif c == 331:  # insert
            ...
            if self.readonly:
                self.beepIfNeeded()
                self.setStatus(
                    "This is a read only line. Text can not be modified.")
            else:
                uchar = self.getunicode(c)
                if not self.insertMode:
                    self.currentLine[self.ptr] = uchar
                else:
                    self.currentLine = u"%s%s%s" % (
                        self.currentLine[:self.ptr], uchar, self.currentLine[self.ptr:])
                    self.ptr += 1
        ...
        log("Readline:handle: currentLine=%s, c=%s, ptr=%d, maxLength=%d" %
            (self.currentLine, c, self.ptr, self.maxLength,))
        # handled keystroke
        self.draw()
        return 1
...
class Editbox(object):
    ...
    def text_delete(self, y, x):
        if y < 0 or x < 0 or y >= len(self.text) or x >= len(self.text[y]):
            return
        del self.text[y][x]
...
#  log("do_command:top ch=%s, y=%d, x=%d" % (ch,y,x))
        if ch == curses.ascii.SOH:         # ^a
            ...
            if x > 0:
                ...
                y -= 1
                # if len(self.text[y])<self.maxx else self.maxx
                x = len(self.text[y])-1
                self.win.move(y, x)
            ...
            x = len(self.text[y]) if len(
                self.text[y]) < self.maxx else self.maxx
            self.win.move(y, x)
...
#  log("edit:draw:top y=%d, x=%d, l=%d" % (y,x,l))
    ...
    def getunicode(self, c):
        ...
        try:
            ...
            pass
        log("getunicode: tc=%s, buf=%s, nc=%d buflen=%d buf[0]=%d" % (
            tc, buf, ord(nc), len(buf), ord(buf[0])))
        return tc
...
# so ["a","b","c"] would become [[0,"a"],[1,"b"],[2,"c"]]
        ...
        if height == None:
            height = (len(self.items)+2) if (len(self.items) +
                                             2) < self.base.maxy-2 else self.base.maxy-2
        self.height = height
...
# if startL%lsitHiehgt==0 then we can clear the screen
        ...
        if self.lastDraw != (startL, endL, [i for i in self.selections]):
            ...
            for idx, itm in enumerate(show):
                ...
                if self.multiple:
                    s = "[%s] %s" % (
                        ("+" if startL+idx in self.selections else "-"), itm[1],)
                else:
                    s = "%s" % (itm[1],)
...
class FileBrowser(Listbox):
    ...
    def makeNewList(self):
        self.l = []
        self.lastScreen = 0
        files = [("d" if os.path.isdir(os.path.join(self.dir, i)) else "f", os.path.join(
            self.dir, i), i) for i in sorted(os.listdir(self.dir))]
