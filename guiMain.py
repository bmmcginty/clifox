...
class gui(forms):
   ...
   """
    ...
    newLineElements = set(
        "FORM,TITLE,HR,LI,TD,H1,H2,H3,H4,H5,H6,BR,P,A,IMG,SOUND,BGSOUND".split(","))
...
# e.g. 2 screens, screen 0 (first screen) has 4 lines, 0-3 inclusive. screen 2, or sn=1, has 4 lines, 0-3 inclusive. line 2 on that second screen is 0,0 0,1 0,2 0,3 1,0 1,1 or 5 (6, because of the zero base)
    def getScreenAbsolutePosition(self, screenNum=None, y=0):
        ...
        while sn < screenNum:
            ...
            sn += 1
        pos += y
        log("getScreenAbsolutePosition:screenNum=%s,y=%s,pos=%s" %
            (str(screenNum), str(y), str(pos),))
        return pos
...
#  log("clifox:res",str(res))
        ...
        for o in res:
            ...
            if o[0] in jm:
                t = jm[o[0]]
                t.ref.vars.update(zip(["num", "nodeName", "nodeValue", "nodeType", "parentNode"], [
                                  o[i] for i in range(1, 6)]))
                if t.ref.vars['parentNode']:
                    t.ref.vars['parentNode'] = jm[t.ref.vars['parentNode']]
                t.ref.vars.update([(o[i], o[i+1])
                                   for i in range(6, len(o), 2)])
                lc.append(t)
            else:
                ...
                v = {"num": num, "nodeName": nn, "tagName": nn, "nodeValue": v,
                     "nodeTypeInvalid": t, "parentNode": jm.get(pn, None), "_flags_skip": 0}
                v.update([(o[j], o[j+1])
                          for j in xrange(6, len(o), 2) if o[j] not in v])
...
#      if count==0:
                        ...
                        closings = [
                            i for i in closings if i.nodeName != "#text"]
...
#    fh.write(str(j)+"\n")
    ...
    def showBookmarks(self):
        ...
        historyService = self.js.Components.classes["@mozilla.org/browser/nav-history-service;1"].getService(
            inhs)
        ...
        bookmarksService = self.js.Components.classes["@mozilla.org/browser/nav-bookmarks-service;1"].getService(
            inbs)
...
#  return rootNode
    ...
    def showScreen(self, screenNum=None, y=None, absoluteY=None, x=None, force=0):
        ...
        if absoluteY == None:
            ...
            absoluteY = self.getScreenAbsolutePosition(
                screenNum=screenNum, y=y)
        else:
            absoluteY = absoluteY
            screenNum = self.getScreenNumber(absoluteY)
            y = self.getLineNumber(absoluteY)
        self.writeScreenCoordsToFile()
        log("showScreen:", "absoluteY:", absoluteY,
            "x:", x, "screen:", screenNum, "y:", y)
    ...
    def searchPage(self, direction="forward"):
        ...
        lines = d.keys()
        if direction == 1:
            lines = [i for i in lines if i > pos] + \
                [i for i in lines if i < pos]+[i for i in lines if i == pos]
            func = "find"
        else:
            lines = [i for i in lines if i < pos][::-1] + \
                [i for i in lines if i > pos][::-1] + \
                [i for i in lines if i == pos]
            direction = -1
            func = "rfind"
        for y in lines:
...
#  nodes=self.nodes_flat #self.iterNodes(self.dom.document)
        ...
        if direction == "forward":
            for yW in xrange(absoluteY, max(self._display.keys())+1):
                if yW not in self._display:
                    continue
                for xW, text, elem in self._display[yW]:
                    if yW == absoluteY and xW <= x:
                        continue
