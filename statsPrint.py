...
#    print i
                try:
                    ln, hits, time, perhit, percentTime, line = re.findall(
                        "^ *([0-9.]+) *([0-9.]+) *([0-9.]+) *([0-9.]+) *([0-9.]+)  (.*)$", i)[0]
                except:
                    continue
# print i; continue
...
#    if not hits.strip(): continue
                fl.append({"ln": int(ln.strip()), "hits": int(hits.strip()), "time": int(time.strip(
                )), "perhit": float(perhit.strip()), "percentTime": float(percentTime.strip()), "line": line})
