...
class JSClass(object):
    def __init__(self, name="", value="", id="", root=0, parent=None, type="object", q=None, vars={}, hostname="localhost", port=4242):
        self.ref = JSReference(name=name, value=value, id=id, root=root, parent=parent,
                               type=type, q=q, vars=vars, hostname=hostname, port=port, proxy=self)
# getattr/setattr
...
#  log("root",r.root,"self",self,"r.id",r.id,"r.map",r.map.keys(),"parent",r.parent)
        ...
        if ret['a'][0] == None:
            return None
        a = JSClass(name=x, root=r.root, parent=self,
                    id=ret['i'], type=ret['t'], value=ret['a'][0])
        ar = a.ref
        if ar.type == "undefined":
            raise AttributeError("%s has no attribute %s" %
                                 (self.ref.name, str(ar.name),))
...
# ~~
        ...
        if ret['t'] not in ("undefined", "array", "object", "function"):
            return ret['a'][0]
        a = JSClass(name=r.name+".result", root=r.root, parent=self,
                    id=ret['i'], type=ret['t'], value=ret['a'][0])
        ar = a.ref
        if ar.type == "undefined":
            return None
...
# an event popped up, stick it in the q.
                if ret['m'] in ("w", "e", "E", "ec"):
                    if ret['m'] in ("e", "E"):
                        x = JSClass(name=ret['t'], id=ret['a']
                                    [0], parent=None, root=self.root)
                        self.map[x.ref.id] = x
                        ret['a'][0] = x
                    self.eventQ.put(ret)
                    continue
...
def initCliFox(hostname="localhost", port=4242, q=None, js=None, ignoreErrors=0):
    ...
    j = JSClass(name="this", value=None, id="jthis", root=0,
                q=eventQ, hostname=hostname, port=port)
    try:
        w = j.ref.eval(js)
#  print "ret:",w
    except Exception, e:
